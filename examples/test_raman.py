...
if __name__ == '__main__':
    ...
    setup.pulse_model = gnlse_main.SechEnvelope(peak_power, duration)
    ...
    raman_models = {
        'Blow-Wood': gnlse_main.raman_blowwood,
        'Hollenbeck-Cantrell': gnlse_main.raman_holltrell,
        'Lin-Agrawal': gnlse_main.raman_linagrawal,
        'No scattering': None
    }
    ...
    for (i, (name, raman_model)) in enumerate(raman_models.items()):
        ...
        plt.title(name)
        gnlse_main.plot_wavelength_vs_distance(solution, WL_range=[500, 1250])

        plt.subplot(2, count, i + 1 + count)
        gnlse_main.plot_delay_vs_distance(solution, time_range=[-0.5, 4])
    plt.tight_layout()
    plt.show()
